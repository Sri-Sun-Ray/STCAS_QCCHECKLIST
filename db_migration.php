...
try {
    ...
    // ==========================================
    // 1. SHIFT S_NO AFTER REMOVAL OF 1.53
    // ==========================================
    // old 1.54 -> new 1.53
    // old 1.55 -> new 1.54
    // ...
    ...
    // Step 1: move 1.54...1.64 to temporary S_no
    ...
    // Step 2: now delete only old removed point 1.53
    $deleted_points = ["11.3", "1.53"];
    $total_deleted = 0;

    foreach ($tables as $table) {
        foreach ($deleted_points as $del_sno) {
            $stmt = $conn->prepare("DELETE FROM $table WHERE S_no = ?");
            if ($stmt) {
                $stmt->bind_param("s", $del_sno);
                $stmt->execute();
    ...
    $stmt = $conn->prepare("DELETE FROM images WHERE s_no = ?");
    $del_img_sno = "1.53";
    if ($stmt) {
        $stmt->bind_param("s", $del_img_sno);
        $stmt->execute();
        $total_deleted += $stmt->affected_rows;
        $stmt->close();
    }

    // Step 3: move TEMP values to final shifted S_no
    for ($i = $SHIFT_START; $i >= $SHIFT_END; $i--) {
        $temp_sno = "TEMP_1.$i";
        $new_sno = "1." . ($i - 1);
    ...
    echo "Deleted $total_deleted obsolete observations/images.<br>";
    echo "Shifted standard S_no values from 1.54..1.64 down by 1 safely.<br>";
    ...
    if ($changed_count > 0) {
        echo "Automatically re-aligned $changed_count custom rows.<br>";
    }
